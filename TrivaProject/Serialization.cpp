...
ServerMessageRequest JsonRequestPacketDeserializer::deserializeServerMessage(const Buffer& buffer)
{
	json data = deserializeRequest(buffer);
	ServerMessageRequest req;
...
Buffer JsonResponsePacketSerializer::serializeResponse(const ServerMessageResponse& resp)
{
	json data;
	data["status"] = resp.status;

	return serializeResponse(SERVER_MESSAGE, to_string(data));
}
