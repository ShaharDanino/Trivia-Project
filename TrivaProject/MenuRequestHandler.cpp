...
bool MenuRequestHandler::isRequestRelevant(const RequestInfo& requestInfo) const
{
    return (requestInfo.requestCode == GET_ROOMS_CODE ||
        ...
        requestInfo.requestCode == SERVER_MESSAGE);
}
...
RequestResult MenuRequestHandler::handleNewRequest(RequestInfo& requestInfo)
{
    ...
    case SERVER_MESSAGE:
        handleServerMessage(requestInfo);
    default:
        return generateErrorResponse("Request Didn't match");
    }
}

RequestResult MenuRequestHandler::handleServerMessage(const RequestInfo& requestInfo)
{
    ServerMessageRequest req = JsonRequestPacketDeserializer::deserializeServerMessage(requestInfo.buffer);
